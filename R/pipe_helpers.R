...
pipe_autoexec <- function(toggle) {
  ...
  if (isTRUE(info[["is_piped"]])) {
    sq_exit <- function(q) if (inherits(q, "sparql_dsl")) sparql_exec(q) else q
    pipeline_on_exit(info$env)
    info$env$.sq_exitfun <- if (toggle) sq_exit else identity
  }
...
pipeline_on_exit <- function(env) {
  ...
  if (exists(".sq_exitfun", envir = env, inherits = FALSE, mode = "function")) {
    return(invisible())
  }
  env$.sq_exitfun <- identity

  res <- NULL

  sq_result <- function(v)
  {
    if (missing(v)) {
      res
    }
    else {
      res <<- `$<-`(v, value, env$.sq_exitfun(v$value))
    }
  }

  makeActiveBinding("result", sq_result, env)
}
